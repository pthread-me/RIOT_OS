...
// macros used to set compression at each layer, 0 to not compress
#define COMPRESS_IP 0
#define COMPRESS_UDP 0
#define COMPRESS_COAP 1
	...
	//	id, up, down, length
		10, 10, 10,
		{
			//	field, 			MO, len, pos,dir, 	val,			MO,				CDA
				...
				{ IP6_DEVIID,	1, 64,	1, BI, 		{0x78, 0x29, 0x50, 0xff, 0xfe, 0x15, 0xe6, 0x56},			&mo_equal,		NOTSENT	   },
				{ IP6_APPPRE,	1, 64,	1, BI,		{0xfe, 0x80,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00},			&mo_equal,		NOTSENT  },
				{ IP6_APPIID,	1, 64,	1, BI, 	    {0x78, 0x29, 0x50, 0xff, 0xfe, 0x15, 0xe6, 0x56},			&mo_equal,		NOTSENT  },
		}
};
...
static const struct schc_coap_rule_t coap = {
    6,6,6,
	{
		{COAP_V,    1, 2, 1,    BI, {COAP_V1},                  &mo_equal,      NOTSENT},
		{COAP_T,    3, 2, 1,    BI, {CT_CON, CT_NON, CT_ACK},   &mo_matchmap,   MAPPINGSENT},
		{COAP_TKL,  1, 4, 1,    BI, {1},                        &mo_equal,      NOTSENT},
        {COAP_C,    2, 8, 1,    BI, {CC_POST, CC_CONTENT},      &mo_matchmap,   MAPPINGSENT},
        {COAP_MID,  1, 16, 1,   BI, {0},                        &mo_ignore,     VALUESENT},
        {COAP_TKN,  16, 8,  1,  BI, {0x00, 0x01, 0x02, 0x03,
                                     0x04, 0x05, 0x06, 0x07,
                                     0x08, 0x09, 0x0A, 0x0B,
                                     0x0C, 0x0D, 0x0E, 0x0F},
...
static const struct schc_coap_rule_t coap_uncompressed = {
    6,6,6,
	{
        ...
        {COAP_MID,  1, 8, 1, BI,   {0},    &mo_ignore, VALUESENT},
        {COAP_TKN,  1, 8, 1, BI, {0},       &mo_ignore, VALUESENT},
    }
};
#endif
...
const struct schc_compression_rule_t server = {
		.rule_id = 0x2,
		.rule_id_size_bits = 8,
...
const struct schc_compression_rule_t client = {
		.rule_id = 0x1,
		.rule_id_size_bits = 8,
